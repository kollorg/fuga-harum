...
# @kollorg/fuga-harum - the node.js worker pool

![CI](https://github.com/jasnell/@kollorg/fuga-harum/workflows/CI/badge.svg)
...
const Piscina = require('@kollorg/fuga-harum');

const @kollorg/fuga-harum = new Piscina({
  filename: path.resolve(__dirname, 'worker.js')
});

(async function() {
  const result = await @kollorg/fuga-harum.run({ a: 4, b: 6 });
...
```js
import { Piscina } from '@kollorg/fuga-harum';
...
(async function() {
  const res = await Promise.all([
    @kollorg/fuga-harum.run({ a: 4, b: 6 }, { name: 'add' }),
    @kollorg/fuga-harum.run({ a: 4, b: 6 }, { name: 'multiply' })
  ]);
})();
```
...
(async function() {
  ...
  try {
    ...
    const task = @kollorg/fuga-harum.run({ a: 4, b: 6 }, { signal });
...
(async function() {
  ...
  try {
    const task = @kollorg/fuga-harum.run({ a: 4, b: 6 }, { signal: ee });
    ee.emit('abort');
    await task;
  } catch (err) {
    console.log('The task was canceled');
...
https://github.com/kollorg/fuga-harum/tree/master/examples
...
* The following optional configuration is supported:
  ...
  * `workerData`: (`any`) Any JavaScript value that can be cloned and made
    available as `require('@kollorg/fuga-harum').workerData`. See [Node.js new Worker options][]
...
```js
const { move } = require('@kollorg/fuga-harum');
...
```js
const { Piscina, FixedQueue } = require('@kollorg/fuga-harum');
const { resolve } = require('path');
...
for (let i = 0; i < 10; i++) {
  @kollorg/fuga-harum.runTask({ data: i }).then((result) => {
    console.log(result);
  }).catch((error) => {
    console.error(error);
  });
...
* add `needsDrain` property ([#368](https://github.com/kollorg/fuga-harum/issues/368)) ([2d49b63](https://github.com/kollorg/fuga-harum/commit/2d49b63368116c172a52e2019648049b4d280162))
* correctly handle process.exit calls outside of a task ([#361](https://github.com/kollorg/fuga-harum/issues/361)) ([8e6d16e](https://github.com/kollorg/fuga-harum/commit/8e6d16e1dc23f8bb39772ed954f6689852ad435f))


#### Bug Fixes

* Fix types for TypeScript 4.7 ([#239](https://github.com/kollorg/fuga-harum/issues/239)) ([a38fb29](https://github.com/kollorg/fuga-harum/commit/a38fb292e8fcc45cc20abab8668f82d908a24dc0))
* use CJS imports ([#374](https://github.com/kollorg/fuga-harum/issues/374)) ([edf8dc4](https://github.com/kollorg/fuga-harum/commit/edf8dc4f1a19e9b49e266109cdb70d9acc86f3ca))
...
* Adds a new `PISCINA_DISABLE_ATOMICS` environment variable as an alternative way of
  disabling Piscina's internal use of the `Atomics` API. (https://github.com/kollorg/fuga-harum/pull/163)
* Fixes a bug with transferable objects. (https://github.com/kollorg/fuga-harum/pull/155)
* Fixes CI issues with TypeScript. (https://github.com/kollorg/fuga-harum/pull/161)

### 3.1.0

* Deprecates `@kollorg/fuga-harum.runTask()`; adds `@kollorg/fuga-harum.run()` as an alternative.
  https://github.com/kollorg/fuga-harum/commit/d7fa24d7515789001f7237ad6ae9ad42d582fc75
...
[`examples/task-queue`]: https://github.com/jasnell/@kollorg/fuga-harum/blob/master/examples/task-queue/index.js
